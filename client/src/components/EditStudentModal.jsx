...
export default function EditStudentModal({ isOpen, onClose, onSave, student }) {
  ...
  useEffect(() => {
    if (student) {
      setFormData({
        ...
        leetcode: student.stats?.leetcode?.username || '',
        hackerrank: student.stats?.hackerrank?.username || '',
        codechef: student.stats?.codechef?.username || '',
        codeforces: student.stats?.codeforces?.username || '',
      });
    }
  }, [student]);
  ...
  return (
      <div className="bg-white scrollbar-hide dark:bg-gray-800 p-6 rounded-lg shadow-lg w-full max-w-xl max-h-[90vh] overflow-y-auto">
        <h2 className="text-xl font-semibold mb-4 text-gray-900 dark:text-white">Edit Student</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          {['name', 'email', 'rollNo', 'leetcode', 'hackerrank', 'codechef', 'codeforces'].map((field) => (
            <div key={field}>
